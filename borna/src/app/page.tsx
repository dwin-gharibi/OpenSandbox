        ...
        <div className="flex items-center gap-3 text-[var(--danger)]">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-1">
              {error}. Make sure the OpenSandbox server is running and your API
              key is configured.
            </p>
          </div>
        </div>
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-[var(--text-secondary)] mt-1">
          Real-time health monitoring for OpenSandbox
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          title="Avg CPU Usage"
          value={`${data.avg_cpu_percent}%`}
          icon={Cpu}
        />
        <StatCard
          title="Avg Memory Usage"
          value={`${data.avg_memory_percent}%`}
          icon={MemoryStick}
        />
        <StatCard
          title="Uptime"
          value={formatUptime(data.uptime_seconds)}
          subtitle={`${data.requests_per_minute} req/min`}
          icon={Clock}
        />
      </div>
